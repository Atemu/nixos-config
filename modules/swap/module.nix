...

{
  ...
  in lib.mkIf (enabledDevices != { }) {
    ...
    boot.kernelParams = lib.mkIf this.zswap.enable [
      "zswap.enabled=1"

      # I think with swap, you need speed first and foremost and compression
      # efficiency is secondary. OTOH, in my testing, zstd is able to store the
      # first 3 pages of random files in my closure in 1 page ~40% of the time
      # vs. only ~10% with lz4 although the bulk is 2 pages in either case. I
      # consider zswap a best-effort speed efficiency anyways though, so speed

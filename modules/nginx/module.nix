...

{
          ...
          { name, config, ... }:
          {
            options = {
              ...
              baseDomain = lib.mkOption {
                default = (
                  if config.onPrimaryDomain then
                    primaryDomain
                  else
                    concatDomain [
              ...
              TLS = {
                enable =
                  lib.mkEnableOption "TLS certificates via ACME"
                  // lib.mkOption {
                    default = true;
                  };
              };
  ...
  config =
    ...
    lib.mkIf (validHosts != { }) {
      ...
      custom.acme.domains = lib.mapAttrs' (
        name: host:
        lib.nameValuePair host.ACMEHost (
          lib.mkIf host.TLS.enable {
